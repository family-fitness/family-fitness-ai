...
막지 않는다 — 비어 있으면 `/readyz` 가 준비되지 않았다고 답한다 (docs/05 §4.2).
...
# 코치 답 문구를 쓰는 쪽. `labeler_backend` 와 같은 모양으로 바꿔 끼운다.
CoachBackend = Literal["claude", "gemini"]
...
class Settings(BaseSettings):
    ...
    # docs/04 §3 — 잰 값은 0.57 (docs/05 §4.2). 여기에 기본값을 두지 않는 이유는,
    # 임베딩 모델을 바꾸고도 다시 재지 않은 채 서비스가 뜨는 길을 막기 위해서다.
    sim_threshold: float | None = Field(default=None, ge=0.0, le=1.0)
    # docs/02 — 영상 라벨 LLM. 기본은 외부 호출이 없는 로컬이다 (AGENTS.md §3).
    labeler_backend: LabelerBackend = "llama"
    llama_server_url: str = "http://127.0.0.1:8081"
    # docs/04 §2.2 — 청크·질의 임베딩 (`bge-m3` · 1024). 색인과 검색이 같은 것을 쓴다.
    embedding_url: str = "http://127.0.0.1:8082"
    # 비우면 백엔드의 기본 모델이다.
    labeler_model: str | None = None
    # docs/05 ④ — `coach/messages` 의 답 문구를 LLM 이 쓰나.
    # **기본은 꺼짐이다.** 꺼져 있으면 찾은 청크에서 발췌해 답하고 외부 호출이 0이다
    # (`make verify` 가 외부에 나가지 않는다). 켜면 LLM 이 쓰고, 실패·마감 초과·
    # 인용 검사 실패면 발췌로 강등한다 (docs/01 §3.1).
    coach_llm: bool = False
    # 어느 LLM 이 쓰나. `LABELER_BACKEND` 와 같은 방식이다 — 호출부는 바뀌지 않는다.
    coach_backend: CoachBackend = "claude"
    # 비우면 고른 백엔드의 기본 모델이다 — 두 벌이 되지 않게.
