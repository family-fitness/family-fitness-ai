...
# 했고, 기준표도 개월 구간으로 나뉜다 (docs/02). 만 5세는
...
def resolve_age_group(age: int, age_unit: AgeUnit) -> AgeGroup | None:
    """계약이 응답에 실을 연령대 (docs/03).

    점수를 낼 수 있는지와는 다른 물음이다 — 어르신과 만 7~10세는 연령대가 정해지되
    `factors` 가 빈 배열로 나간다 (docs/03).

    **`세` 로 온 만 4~6세는 `None` 이다** — 유아기는 개월로 받는다 (docs/03).

    **만 7~10세는 공식 구간에 없다.** 측정이 0건이라 그렇다. 가장 가까운 `유소년` 으로
    둔다 — 확정 전 기본값이다 (docs/02).
    """
    if age_unit == "개월":
