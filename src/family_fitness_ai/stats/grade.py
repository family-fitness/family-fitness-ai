...
시트를 대조해 옮겼다. `../../docs/02` §5.2 의 서술은 이보다 거칠다 (docs/02 §5.3).
...
# 문턱이 아니라 구간이라 `body_composition_ranges.csv` 에 따로 있다 (docs/02 §5.3).
...
def _notes(body: list[BodyRange]) -> tuple[str, ...]:
    ...
    if body:
        return ()
    return ("신체조성 구간이 없어 3등급 판정에서 빠졌다 (docs/02 §5.3)",)
