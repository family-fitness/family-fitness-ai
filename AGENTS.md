...
| 최종 목적 · 백엔드 구현과 어긋나는 곳 | `docs/05-코치-설계.md` |
...
팀원 것이다.** 그쪽에 필요한 변경이 보이면 고치지 말고 알린다. 차이는 `docs/05` 에 적는다.
...
- **원자료를 커밋하지 않는다.** 목록은 `data/manifest.csv` (`docs/02`)
...
**아래의 모든 명령과 `docs/05` 의 실행 예시는 venv 를 켠 상태를 전제한다.**
...
진행 상태의 정본은 `docs/05-코치-설계.md` 다. 코드를 새로 넣을 때는 그 구역의
문서를 먼저 읽고, 문서에 없는 규칙을 코드로 만들지 않는다.
